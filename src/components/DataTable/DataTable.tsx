...
export const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
  const { data, setReloadState } = props;
  console.log(data);
  const renderTable = () => {
    return data.map((item) => {
      return (
        <tr
          key={item._id}
          className="border-b hover:text-white hover:bg-gray-600 bg-white text-gray-800"
        >
          ...
          <td className="py-3 px-6">{item.click}</td>
    ...
    <div className="container mx-auto pt-2 pb-10">
      <div className="relative overflow-x-auto shadow-sm sm:rounded-lg">
        <table className="w-full table-fixed text-sm text-left rtl:text-right text-gray-500">
          <thead className="text-md uppercase text-gray-800 bg-gray-300">
            <tr>
